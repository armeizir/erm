"""Repair SPI Juni 2026 V5 (dynamic report resolver).
...
DRY RUN:
  python monthly_report/scripts/repair_spi_june_2026_v5.py

APPLY:
  python monthly_report/scripts/repair_spi_june_2026_v5.py --apply
"""
...
def get_report(ref):
    # Resolve SPI June 2026 report dynamically; never trust a DB primary key.
    candidates = list(
    ...
    if "SPI" not in str(report.reassessment.unit_bisnis).upper():
        raise RuntimeError(f"Unit report bukan SPI: {report.reassessment.unit_bisnis}")
    return report
...
def validate_mapping(ref, report):
    ...
    for item in ref["items"]:
        risk = ReAssessmentItem.objects.filter(
            pk=item["re_id"], summary_id=report.reassessment_id
        ).first()
        if not risk:
...
def audit(ref, report, rows):
    ...
    print("=" * 140)
    print("AUDIT SPI JUNI 2026 V5 - TARGET Q2 + ACTUAL JUNI")
    print("=" * 140)
...
def validate_after(ref, report, rows):
    ...
    if errors:
        raise RuntimeError("VALIDASI V5 GAGAL:\n- " + "\n- ".join(errors))

    print("\nVALIDASI DATA V5 BERHASIL")
