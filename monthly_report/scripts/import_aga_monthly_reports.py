...
BASE_DIR = Path("/Users/armeizir/Downloads/2026/4. Bid. Niaga")
...
FILES = [
    ImportFile(2, BASE_DIR / "Laporan Realisasi Februari 2026 BID AGA - AMS.xlsx"),
    ImportFile(3, BASE_DIR / "Laporan Realisasi Maret 2026 BID AGA.xlsx"),
    ImportFile(4, BASE_DIR / "Laporan Realisasi April 2026 BID AGA.xlsx"),
]
...
def decimal_or_none(value):
    if value in (None, ""):
        return None
    if isinstance(value, str) and value.strip() in {"-", "No Data"}:
        return None
    try:
...
def build_event_map(reassessment):
    ...
    for item in ReAssessmentItem.objects.filter(summary=reassessment):
        result[normalize(item.peristiwa_risiko)] = item
    return result
...
def import_sheet_iiia(workbook, report, event_map, month, skipped):
    ...
    for row_idx, row in read_rows(ws, 11):
        ...
        risk_event = event_map.get(normalize(event_name))
        if not risk_event:
            skipped.append(
                {
                    "sheet": "III.A",
                    ...
                    "no": no_risiko,
                    "event": event_name,
                    "reason": "Peristiwa risiko tidak ditemukan di Profil Risiko BID AGA.",
                }
            )
            continue
...
def import_sheet_iiib(workbook, report, event_map, month, skipped):
    ...
    for row_idx, row in read_rows(ws, 10):
        ...
        item.progress_pelaksanaan_percent = progress_percent(
...
def import_sheet_iiie(workbook, report, skipped):
    ws = workbook["III.E"]
    ...
    for row_idx, row in read_rows(ws, 8):
        ...
        if not name:
            continue
        if "harga jual" not in normalize(name):
            skipped.append(
                {
                    "sheet": "III.E",
                    "row": row_idx,
                    "event": name,
                    "reason": "Tidak diimpor otomatis karena bukan kejadian AGA/Niaga yang jelas.",
                }
            )
            continue
...
def run():
    ...
    reassessment = ReAssessmentSummary.objects.get(pk=1)
    ...
    with transaction.atomic():
        for import_file in FILES:
            ...
            report, _ = MonthlyRiskReport.objects.update_or_create(
                ...
                versi=1,
                defaults={
                    "kode": f"MRR-BIDAGA-{tahun_buku.tahun}-{import_file.month:02d}",
                    "tahun_buku": tahun_buku,
                    "status": "draft",
            ...
            iiie_count = import_sheet_iiie(workbook, report, skipped)
