...
BASE_DIR = Path("/Users/armeizir/Downloads/2026/9. SPI")
REASSESSMENT_ID = 11
...
FILES = [
    ImportFile(2, BASE_DIR / "Laporan Mitigasi RIsiko dan KRI Feb 2026 Rev 02.xlsx"),
    ImportFile(3, BASE_DIR / "Laporan Mitigasi RIsiko dan KRI Maret 2026.xlsx"),
    ImportFile(4, BASE_DIR / "Laporan Mitigasi RIsiko dan KRI April  2026.xlsx"),
]
...
def build_event_maps(reassessment):
    by_number = {}
    by_name = {}
    for item in ReAssessmentItem.objects.filter(summary=reassessment):
        if item.no_risiko is not None:
            by_number[int(item.no_risiko)] = item
        by_name.setdefault(normalize(item.peristiwa_risiko), item)
    return by_number, by_name


def resolve_risk_event(row, by_number, by_name):
    number = risk_number(row[1] if len(row) > 1 else None)
    if number in by_number:
        return by_number[number]
    return by_name.get(normalize(row[2] if len(row) > 2 else None))
...
def import_sheet_iiia(workbook, report, by_number, by_name, month, skipped):
    ws = workbook["III.A"]
    ...
    for row_idx, row in read_rows(ws, 11):
        ...
        risk_event = resolve_risk_event(row, by_number, by_name)
        if not risk_event:
            skipped.append(
                {
                    "sheet": "III.A",
                    ...
                    "no": no_risiko,
                    "event": event_name,
                    "reason": "Peristiwa risiko tidak ditemukan di Profil Risiko SPI.",
                }
            )
            continue
...
def import_sheet_iiib(workbook, report, by_number, by_name, month, skipped):
    ws = workbook["III.B"]
    q = selected_quarter(month)
...
def import_sheet_iiie(workbook, report, by_name, skipped):
    ...
    for row_idx, row in read_rows(ws, 8):
        ...
        if not name or "tidak terjadi loss event" in normalized_name:
            continue
            ...
            skipped.append(
                {
                    ...
                    "row": row_idx,
                    "event": name,
                    "reason": "Tidak diimpor otomatis karena tidak terkait item risiko SPI.",
                }
            )
            continue
...
def run():
    ...
    by_number, by_name = build_event_maps(reassessment)
    summary = []
    all_skipped = []
    ...
    with transaction.atomic():
        for import_file in FILES:
            ...
            report, _ = MonthlyRiskReport.objects.update_or_create(
                ...
                versi=1,
                defaults={
                    "kode": f"MRR-SPI-{tahun_buku.tahun}-{import_file.month:02d}",
                    "tahun_buku": tahun_buku,
                    "status": "draft",
