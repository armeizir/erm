...
def generate_rule_based_ai_insight_for_multi_metric_result(result, user_context=""):
    ...
    # It is a concise draft derived from the already-generated AI Insight.
    action_lines = [
        line.strip()
    ...
    management_decision_draft = (
        f"{str(executive_summary or '').strip()}\n\n"
        "Arah tindak lanjut yang direkomendasikan:\n"
        f"{action_summary or str(recommended_actions or '').strip()}\n\n"
        "Status: AI Draft — wajib direview dan ditetapkan oleh manajemen."
    ).strip()
