...
class MonteCarloKorporatConfigAdmin(admin.ModelAdmin):
    ...
    def run_button(self, obj):
        ...
        url = reverse(
            f"{self.admin_site.name}:corporate_risk_montecarlo_run",
            args=[obj.pk],
        )

        return format_html(
            '<a class="button" href="{}">Jalankan Monte Carlo</a>',
            url,
        )
    ...
    def run_monte_carlo_view(self, request, config_id, *args, **kwargs):
        ...
        try:
            ...
            return redirect(
                reverse(
                    f"{self.admin_site.name}:corporate_risk_montecarlokorporatresult_change",
                    args=[result.pk],
                )
            )
...
class MonteCarloKorporatResultAdmin(admin.ModelAdmin):
    ...
    change_list_template = (
        "admin/corporate_risk/montecarlokorporatresult/change_list.html"
    )
    ...
    def status_badge(self, obj):
        if obj.status_hasil == "AMAN":
            color = "#28a745"
        elif obj.status_hasil == "WASPADA":
            color = "#ffc107"
        else:
            color = "#dc3545"

        return format_html(
            '<span style="background:{};color:white;padding:5px 10px;border-radius:12px;font-weight:bold;">{}</span>',
            color,
            obj.status_hasil,
        )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                ...
                name="corporate_risk_montecarlo_export_excel",
            ),
            path(
    ...
    def export_excel(self, request):
        ...
        queryset = MonteCarloKorporatResult.objects.all().order_by("-created_at")

        for obj in queryset:
            ws.append([
                str(obj.corporate_risk_item),
                str(obj.forecast_periode),
                obj.metric_name,
                float(obj.mean_value or 0),
                float(obj.p80_value or 0),
                float(obj.probability_meet_target or 0),
                obj.status_hasil,
        ...
        response["Content-Disposition"] = (
            'attachment; filename="monte_carlo_result.xlsx"'
        )
...
class RiskMetricAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        ...
        "is_active",
        "rkap_item__tahun",
        "rkap_item__subkategori",
        "rkap_item__kategori",
    )

    search_fields = (
        "corporate_risk_item__peristiwa_risiko",
        "corporate_risk_item__kode",
        "name",
        "unit",
        "rkap_item__kode",
        "rkap_item__sasaran",
    ...
    fieldsets = (
        ...
        ("Prediksi Risiko / Target RKAP", {
            ...
            "description": (
                "Centang Metric Target Utama. "
                "Pilih Sumber Target RKAP dari data Lampiran 2. "
                "Kosongkan Target RKAP Override jika ingin target otomatis diambil dari tabel RKAP."
            ),
        }),
    )
    ...
    @admin.display(description="Target RKAP Final")
    def target_rkap_final(self, obj):
        if obj.target_value not in (None, ""):
            return obj.target_value

        if obj.rkap_item and obj.rkap_item.target is not None:
            return obj.rkap_item.target
...
class MultiMetricMonteCarloResultAdmin(admin.ModelAdmin):
    change_list_template = "admin/corporate_risk/multimetricmontecarloresult/change_list.html"

    change_list_template = (
        "admin/corporate_risk/multimetricmontecarloresult/change_list.html"
    )
    ...
    def requires_mitigation_badge(self, obj):
    ...
    requires_mitigation_badge.short_description = "Mitigasi"
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "export-excel/",
                self.admin_site.admin_view(self.export_excel),
                name="corporate_risk_multimetric_export_excel",
            ),
        ]
    ...
    def export_excel(self, request):
        ...
        headers = [
            ...
            "P80 Score",
            "Target Status",
            "Probability Achieve Target",
            "Probability Not Achieve Target",
            "Requires Mitigation",
            "Status Hasil",
            "Dibuat Pada",
        ]
        ...
        for obj in queryset:
            ws.append([
                str(obj.corporate_risk_item),
                str(obj.forecast_periode),
                float(obj.composite_score or 0),
                float(obj.p80_score or 0),
                obj.target_status,
                float(obj.probability_achieve_target or 0),
                float(obj.probability_not_achieve_target or 0),
                "Perlu Mitigasi" if obj.requires_mitigation else "Tidak Perlu Mitigasi",
                obj.status_hasil,
                obj.created_at.strftime("%Y-%m-%d %H:%M:%S") if obj.created_at else "",
            ])
        ...
        response["Content-Disposition"] = 'attachment; filename="multi_metric_monte_carlo_result.xlsx"'
        wb.save(response)
        return response
    ...
    def generate_ai_button(self, obj):
        ...
        return format_html(
            '<a class="button" href="{}">Generate AI Insight</a>',
            url,
        )
